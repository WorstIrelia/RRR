...
Board::Board(int r, int c) {
    ...
    vertical_walls = std::vector<std::vector<bool> >(
            rows, std::vector<bool>(cols + 1, false));
    horizontal_walls = std::vector<std::vector<bool> >(
            rows + 1, std::vector<bool>(cols, false));
...
void Board::resetspot(const Position& p) {
    assert(p.row >= 1 && p.row <= rows);
    assert(p.col >= 1 && p.col <= cols);
...
void Board::print() {
    ...
    for (int i = 0; i <= rows; i++) {
        ...
        for (double j = 1; j <= cols; j++) {
            (getHorizontalWall(i + 0.5, j)) ? std::cout << "----"
                                            : std::cout << "    ";
            std::cout << "+";
        }
        std::cout << "\n";
...
// My code starts here
std::vector<std::string> Board::directions = {"north", "east", "south", "west"};

std::vector<Board::dir_step> Board::dir_steps = {
        {-1, 0}, {0, 1}, {1, 0}, {0, -1}};

bool Board::getWall(double x, double y) {
    if ((x - floor(x)) < 1e-6) {
        return getVerticalWall((int)x, y);
    }
    return getHorizontalWall(x, (int)y);
}
// ==================================================================
// ==================================================================
